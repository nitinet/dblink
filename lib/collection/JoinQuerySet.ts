...
class JoinQuerySet<T extends Object, U extends Object> extends IQuerySet<T & U> {
   ...
   */
  listPlainAndCount(keys: (keyof T | keyof U)[]): Promise<{ count: number; values: Partial<T & U>[] }> {
    throw new Error('Method not implemented.');
  }
   ...
   * @returns {Promise<Array<T & U>>}
   */
  async mapData(input: ResultSet): Promise<Array<T & U>> {
   ...
   * @returns {IQuerySet<V>}
   */
  select<V extends Object>(EntityType: exprBuilder.types.IEntityType<V>): IQuerySet<V> {
    // this.stat.command = sql.types.Command.SELECT;
   ...
   */
  where(param: exprBuilder.types.IWhereFunc<exprBuilder.WhereExprBuilder<T & U>>, ...args: any[]): IQuerySet<T & U> {
    // let res = null;
   ...
   * @returns {IQuerySet<T & U>}
   */
  groupBy(param: exprBuilder.types.IArrFieldFunc<exprBuilder.GroupExprBuilder<T & U>>): IQuerySet<T & U> {
    // let res = null;
    // if (param && param instanceof Function) {
    // 	//TODO: fix join fieldMap
   ...
   * @returns {IQuerySet<T & U>}
   */
  orderBy(param: exprBuilder.types.IArrFieldFunc<exprBuilder.OrderExprBuilder<T & U>>): IQuerySet<T & U> {
    // let res = null;
    // if (param && param instanceof Function) {
    // 	//TODO: fix join fieldMap
    // 	let mainFieldMap = this.context.tableSetMap.get(null)?.fieldMap;
